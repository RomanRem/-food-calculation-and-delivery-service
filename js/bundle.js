...
function calc() {
  ...
  let sex, height, weight, age, ratio; //логика локального хранилища
  ...
  getStaticInfo('.calculating__choose_big div', 'calculating__choose-item_active'); //обработка инпутов в формах ввода

  function getDynamicInfo(selector) {
    const input = document.querySelector(selector);
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        //алерт при некорректном вводе c использованием регулярки
  ...
  class MenuCard {
    ...
    render() {
      ...
      if (this.classes.length === 0) {
        //добавление класса по умолчанию в случае его отсутствия
        this.el = 'menu__item';
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
            ...
            margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); //переписано под использование Fetch API
...
function slider({
  ...
  let offset = 0;
  let slideIndex = 1; // добавление карусельки
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2021-12-12');
